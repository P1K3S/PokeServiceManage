...
start "backend" /D "%ROOT_DIR%\server" cmd /k "chcp 65001 >nul && set NO_COLOR=1 && server.exe"
timeout /t 3 /nobreak >nul

echo [5/5] Starting frontend (port 5173)...
start "frontend" /D "%ROOT_DIR%\web" cmd /k "chcp 65001 >nul && set NO_COLOR=1 && npm run dev"
